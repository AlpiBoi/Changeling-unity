...
public class pinkPlayerAttack : MonoBehaviour
{
    ...
    {
        pinkProjectile curProjectile;
        if (!sr.flipX)
        {
            curProjectile = Instantiate(projectilePrefab, rightSpawn.position, Quaternion.identity).GetComponent<pinkProjectile>();
            curProjectile.SetVelocity(initShotVelocity);
        }
        else
        {
            curProjectile = Instantiate(projectilePrefab, leftSpawn.position, Quaternion.identity).GetComponent<pinkProjectile>();
            curProjectile.SetVelocity(-initShotVelocity);
        }
    }
