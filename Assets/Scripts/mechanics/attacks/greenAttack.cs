...
public class greenAttack : MonoBehaviour
{
    ...
    public void grenAttack()
    {
        greenProjectile curProjectile;
        if (!sr.flipX)
        {
            curProjectile = Instantiate(projectilePrefab, rightSpawn.position, Quaternion.identity).GetComponent<greenProjectile>();
            curProjectile.SetVelocity(initShotVelocity);
        }
        else
        {
            curProjectile = Instantiate(projectilePrefab, leftSpawn.position, Quaternion.identity).GetComponent<greenProjectile>();
            curProjectile.SetVelocity(-initShotVelocity);
        }
    }
