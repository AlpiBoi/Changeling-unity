...
public class WhiteAttack : MonoBehaviour
{
    ...
    public void WhitAttack()
    {
        whiteProjectile curProjectile;
        if (!sr.flipX)
        {
            curProjectile = Instantiate(projectilePrefab, rightSpawn.position, Quaternion.identity).GetComponent<whiteProjectile>();
            curProjectile.SetVelocity(initShotVelocity);
        }
        else
        {
            curProjectile = Instantiate(projectilePrefab, leftSpawn.position, Quaternion.identity).GetComponent<whiteProjectile>();
            curProjectile.SetVelocity(-initShotVelocity);
        }
    }
}
